...
const SOCKET_SERVER_URL = resolveServerUrl(import.meta.env.VITE_SOCKET_URL);
...
function getOrCreateSocket(auth) {
  ...
  if (!socketInstance) {
    socketInstance = io(SOCKET_SERVER_URL, {
      path: SOCKET_PATH,
      auth,
      // Keep the default polling-first transport order so Vite can proxy the
      // initial Engine.IO handshake before any websocket upgrade.
...
function incidentLogDescription(data, verb = 'started on') {
  const incidentType = getIncidentLabel(data?.incident_type || 'incident');
  const subject = incidentSubject(data);
  const grievance = data?.dominant_grievance ? ` Main cause: ${getGrievanceLabel(data.dominant_grievance)}.` : '';
...
function buildUnrestOverlayEvent(data, fallbackType = 'uprising') {
  ...
  return {
    ...
    description: incidentType === 'revolution'
      ? `The union took control in ${data?.region || 'the territory'}.`
      : `${getIncidentLabel(incidentType)} broke out on ${incidentSubject(data)}.`,
...
export function useSocket({ roomCode, matchId = null, playerId, enabled = true } = {}) {
  ...
  const waitForSocketReady = useCallback((timeoutMs = SOCKET_ACK_TIMEOUT_MS) => {
  ...
  const emit = useCallback((event, data) => {
    ...
    if (!socket.connected && !socket.active && typeof socket.connect === 'function') {
      socket.connect();
    }
  ...
  const emitWithAck = useCallback((event, data, timeoutMs = SOCKET_ACK_TIMEOUT_MS) => {
    return waitForSocketReady(timeoutMs).then((socket) => {
      const activeMatchId = matchId ?? useGameStore.getState().matchId;
      const payload = activeMatchId ? { ...data, match_id: activeMatchId } : data;
  ...
  const plotStart = useCallback((data = {}) => emitWithAck('plot_start', data), [emitWithAck]);
  const plotAction = useCallback((data = {}) => emitWithAck('plot_action', data), [emitWithAck]);

...
const LOBBY_TARGET_LABELS = {
  ...
  stabilization_fund: 'Stabilization fund',
  bailout_enable: 'Enable bailouts',
...
function formatLobbyTargets(targets = []) {
  const normalizedTargets = dedupeLabels(targets.map((target) => LOBBY_TARGET_LABELS[target] || humanize(target)), 3);
...
function MetricCard({ label, value, tone = 'text-slate-100' }) {
  return (
...
function PropertyDetailPanel({ property }) {
  ...
  const statusLabel = property.incident_type
    ? SEVERITY_LABELS[property.incident_type] || humanize(property.incident_type)
    : humanize(statusKey);
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <MetricCard label="Tension" value={`${percent(property.tension)}%`} tone={percent(property.tension) >= 80 ? 'text-rose-300' : 'text-slate-100'} />
        <MetricCard label="Territory Instability" value={`${percent(property.territory_instability)}%`} tone={percent(property.territory_instability) >= 75 ? 'text-orange-300' : 'text-slate-100'} />
      ...
      <div className="mt-5 space-y-4">
        <div>
          <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Why pressure is building</p>
            ...
            {(property.reason_breakdown || []).map((reason) => (
              <div key={`${property.property_id}-${reason.key}`} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-3">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-semibold text-slate-100">{reason.label || humanize(reason.key)}</p>
        ...
        {lobbyBacklash.length > 0 && (
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Lobby Backlash</p>
            <div className="mt-3 space-y-2">
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.22em] text-slate-500">What can be done</p>
...
export default function StabilityPanelModal({ onClose, onSubmitNegotiation, onApplyEmergencyReform }) {
      ...
      .map((player) => {
        ...
        for (const property of relevantProperties) {
          for (const reason of property.reason_breakdown || []) {
            ...
            const existing = reasonTotals[key] || { key, label: reason.label || humanize(reason.key), points: 0 };
  ...
  const renderGrievanceMix = () => (
    <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5">
      <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Grievance Mix</p>
      ...
      <div className="space-y-6">
            ...
            {(territories || []).slice(0, 6).map((territory) => (
              ...
              >
                ...
                <p className="mt-3 text-[11px] uppercase tracking-[0.18em] text-slate-500">
                  {(territory.dominant_grievances || []).slice(0, 2).map(humanize).join(' • ') || 'No dominant grievance'}
                </p>
              </button>
            ))}
          </div>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Properties Close To Revolt</p>
              <h3 className="mt-2 text-lg font-semibold text-slate-100">Owner and property risk table</h3>
            </div>
          ...
          <div className="mt-4 space-y-2">
            ...
            {riskyProperties.map((property) => (
              ...
              >
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Pressure</p>
                  <p className="mt-1 text-sm text-slate-200">{percent(property.tension)} tension</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Territory</p>
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Why</p>
                  <p className="mt-1 text-sm text-slate-200">
                    {property.reason_breakdown?.[0]?.label || humanize(property.dominant_grievance || 'mixed_grievance')}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-slate-500">What Can Be Done</p>
                  <p className="mt-1 text-sm text-slate-200">
    ...
    <>
      <div className="space-y-6">
        <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Active Incidents</p>
              <h3 className="mt-2 text-lg font-semibold text-slate-100">Negotiation and containment</h3>
            </div>
            {myPlayer && (
              ...
              return (
                <div key={incident.incident_id || incident.property_id} className={`rounded-3xl border p-4 ${propertyStyle}`}>
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.18em]">
                          {SEVERITY_LABELS[incident.incident_type] || incident.incident_type}
                        </span>
                        <span className="text-sm font-semibold text-slate-100">{incident.property_name}</span>
                      </div>
                      <p className="mt-2 text-sm text-slate-300">
                        {incident.region || 'Unassigned'} • tension {percent(incident.tension)} • {humanize(incident.dominant_grievance || 'mixed grievance')}
                      </p>
                      <p className="mt-1 text-xs text-slate-400">
                  ...
                  <div className="mt-4 grid gap-4 lg:grid-cols-[1fr,auto] lg:items-end">
                    <div>
                      <div className="flex items-center justify-between text-xs text-slate-400">
                        <span>Negotiation coverage</span>
                        <span>{coverage}%</span>
                      </div>
                      <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-800">
                        <div
                      ...
                      <p className="mt-2 text-xs text-slate-300">Projected relief if funded this round: {projectedReliefLabel(incident)}</p>
                      {incident.spread_block_active && (
                        <p className="mt-2 text-xs font-semibold text-cyan-100">Spread block is currently active for this incident.</p>
                      )}
                    </div>
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-300">
            <p>
              Rage is the macro pressure meter. Territory instability concentrates that pressure into specific owner-region blocs.
            </p>
            <p>
              Wealth-concentrated anti-relief lobbying now feeds local backlash before the policy even resolves, especially when the table already has distressed players.
            </p>
            <p>
              Full negotiation coverage still shortens incidents and blocks spread, but systemic pressure will persist if the same owner keeps funding harmful welfare or bailout cuts.
            </p>
          </div>
        </section>
      </div>
      ...
      <div className="flex h-full max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-[28px] border border-slate-800 bg-slate-900 shadow-2xl shadow-black/40">
        <div className="flex items-start justify-between border-b border-slate-800 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Backend-authoritative unrest, negotiation coverage, matching lobby relief, and union contagion.
            </p>
          </div>
          <button
            type="button"

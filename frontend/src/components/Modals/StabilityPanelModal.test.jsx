...
describe('StabilityPanelModal', () => {
  ...
  it('renders national stability metrics and incident cards', () => {
    ...
    expect(screen.getByText('Public Anger')).toBeInTheDocument();
    expect(screen.getByText('Properties Under Pressure')).toBeInTheDocument();
  ...
  it('submits negotiation funding through the provided callback', () => {
    ...
    expect(screen.getByText(/What happens if you fund it now:/)).toBeInTheDocument();
  ...
  it('lets you select a player and inspect their property instability breakdown', () => {
    ...
    expect(screen.getAllByText('A powerful owner is pushing harsh policies').length).toBeGreaterThan(0);
  });
});
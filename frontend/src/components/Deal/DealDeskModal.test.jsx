...
describe('DealDeskModal', () => {
  ...
  it('keeps the counter draft visible when the investor cannot fund the escrow', async () => {
    ...
    expect(await screen.findByText('Clause 1: ShavingCream cannot currently fund $150.')).toBeInTheDocument();
    expect(onCounter).not.toHaveBeenCalled();
    expect(screen.getByDisplayValue('Private equity bridge')).toBeInTheDocument();

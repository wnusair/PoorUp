import { getClauseLabel, getClauseTone, getDealCounterpartyName, sortDealsForPlayer, summarizeClause } from '../../utils/deals';

export default function ActiveDealsPanel({ deals, players, properties, myPlayerId, selectedDealId, onSelect }) {
                  ...
                  {deal.clauses
                    ...
                    .map((clause) => (
                      <div key={`${deal.id}-${clause.id}-investment`} className="rounded-lg border border-gray-700 bg-gray-900/80 px-2.5 py-2">
                        <p>Escrow: ${(Number((clause?.config?.escrow_remaining ?? clause?.config?.escrow_amount) || 0)).toFixed(0)}</p>
                        <p>Payout: ${(Number(clause?.config?.payout_to_date || 0)).toFixed(0)} / ${(Number(clause?.config?.max_payout || 0)).toFixed(0)}</p>
                      </div>
                    ))}
                </div>
              )}

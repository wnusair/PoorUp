...
export function formatDealDeadline(deadline = {}) {
  ...
  const labels = {
    beneficiary_landings_on_grantor: remaining === 1 ? '1 landing left' : `${remaining} landings left`,
    beneficiary_turns: remaining === 1 ? '1 beneficiary turn left' : `${remaining} beneficiary turns left`,
    grantor_turns: remaining === 1 ? '1 grantor turn left' : `${remaining} grantor turns left`,
    full_rounds: remaining === 1 ? '1 round left' : `${remaining} rounds left`,
    beneficiary_rotations: remaining === 1 ? '1 rotation left' : `${remaining} rotations left`,
  };
...
export function summarizeClause(clause, myPlayerId, players = [], properties = {}) {
  ...
  const escrowRemaining = Number((clause?.config?.escrow_remaining ?? escrowAmount) || 0);
  ...
  const scopeLabel = scopedProperties.length > 0
    ...
    : 'selected assets';
  return `${grantorName} escrows $${escrowRemaining.toFixed(0)} / $${escrowAmount.toFixed(0)} for ${beneficiaryName} on ${scopeLabel}, taking ${profitSharePercent}% of funded rent.`;
}

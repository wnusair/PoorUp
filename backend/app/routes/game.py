...
def propose_trade(match_id):
    ...
    payload, error = normalize_trade_request_payload(match_id, request.get_json(silent=True) or {})
    if error:
    ...
    log_and_broadcast(
        gs, "trade_proposed",
        f"{player.get('username')} proposed a trade to {receiver.get('username')}{pledge_suffix}",
        match_id, redis_client, socketio, player_id=mp.id,
    )
...
def respond_to_trade(match_id, trade_id):
    ...
    if action == "accept":
        ...
        payload, error = normalize_trade_request_payload(match_id, counter_data)
        if error:
            return jsonify({"error": error}), 400
...
def submit_lobby(match_id):
    ...
    for item in requested_contributions:
        ...
        if not policy:
            return jsonify({"error": "Select a valid lobbying target."}), 400

        grouped = grouped_contributions.get(policy.id)
        if grouped is None:
            grouped = {"policy": policy, "contribution": 0.0}
    ...
    for item in normalized_contributions:
        ...
        response_contributions.append({
            "axis": item.get("axis") or policy.to_dict().get("axis"),
            "direction": item.get("direction") or policy.to_dict().get("direction"),
            "contribution": contribution,
...
def pay_jail_bail(match_id):
    ...
    for p in gs["players"]:
        p = dict(p)
        if p["id"] == mp.id:
            p["balance"] = round(float(p["balance"]) - bail, 2)
            p["is_jailed"] = False
            p["jail_turns_remaining"] = 0
        updated_players.append(p)
    ...
    log_and_broadcast(
        gs, "jail_released",
        f"{player.get('username')} paid ${bail:.2f} bail and was released from jail.",
        match_id, redis_client, socketio, player_id=mp.id,
    )
    persist_game_state(gs, match_id, redis_client)
    return jsonify({"state": gs}), 200

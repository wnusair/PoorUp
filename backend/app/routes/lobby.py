"""
Lobby routes: create, join, view, update settings, start game.
"""
import json
...
def update_settings(room_code):
    ...
    from app import socketio
    socketio.emit("lobby_update", _lobby_state(match), room=room_code.upper())
...
def patch_bot(room_code, player_id):
    ...
    return jsonify({"message": "Bot updated.", "player": bot_player.to_dict()}), 200
...
def delete_bot(room_code, player_id):
    ...
    return jsonify({"message": "Bot removed."}), 200

...
def _resolve_join_match(room_code: str, match_id):
    normalized_room_code = (room_code or "").upper()
    ...
    if normalized_room_code:
        return Match.query.filter_by(room_code=normalized_room_code).first()
...
def handle_join_room(data):
    ...
    match = _resolve_join_match(room_code, match_id)
    ...
    user_id = session.get("user_id")
    if user_id and match is not None:
...
def handle_select_color(data):
    ...
    return {"ok": True, "player_id": mp.id, "color_hex": color_hex, "lobby": lobby_state}
...
def handle_submit_lobby(data):
    ...
    if match_id <= 0:
        emit("error", {"message": "A valid match is required."})
        return
    ...
    if not settings.get("lobbying_enabled", True):
        emit("error", {"message": "Lobbying is disabled."})
        return
    ...
    for item in requested_contributions:
        ...
        if policy_error:
            emit("error", {"message": policy_error})
            return
    ...
    if not normalized_contributions:
        emit("error", {"message": "A positive contribution is required."})
        return

    player = next((p for p in gs["players"] if p["id"] == mp.id), None)
    total_contribution = round(sum(item["contribution"] for item in normalized_contributions), 2)
    if float((player or {}).get("balance", 0)) < total_contribution:
        emit("error", {"message": "Insufficient funds."})
        return

    updated_players = []
    updated_player = None
    for state_player in gs.get("players", []):
        state_player = dict(state_player)
        if state_player.get("id") == mp.id:
            state_player["balance"] = round(float(state_player.get("balance", 0) or 0) - total_contribution, 2)
    ...
    mp.balance = round(float(mp.balance or 0) - total_contribution, 2)
...
@socketio.on("plot_start")
def handle_plot_start(data):
    user_id = session.get("user_id")
    if not user_id:
        return {"ok": False, "error": "You must be signed in to found the communist plot."}
    ...
    mp = MatchPlayer.query.filter_by(match_id=match_id, user_id=user_id).first()
    if not mp or mp.is_bankrupt:
        return {"ok": False, "error": "You cannot found the communist plot in this match."}
    if gs.get("current_player_id") != mp.id:
        emit("error", {"message": "You may only found the communist plot on your turn."})
        return {"ok": False, "error": "You may only found the communist plot on your turn."}

    try:
        gs, result = start_communist_plot(gs, player_id=mp.id)
    except ValueError as exc:
        emit("error", {"message": str(exc)})
    ...
    gs = log_and_broadcast(
        gs,
        "plot_action",
        result.get("summary") or "The communist plot was founded.",
        match_id,
        redis_client,
        socketio,
        player_id=mp.id,
...
@socketio.on("plot_action")
def handle_plot_action(data):
    user_id = session.get("user_id")
    if not user_id:
        return {"ok": False, "error": "You must be signed in to use plot actions."}
    ...
    mp = MatchPlayer.query.filter_by(match_id=match_id, user_id=user_id).first()
    if not mp or mp.is_bankrupt:
        return {"ok": False, "error": "You cannot use plot actions in this match."}
    if gs.get("current_player_id") != mp.id:
        emit("error", {"message": "Direct plot actions may only be used on your turn."})
        return {"ok": False, "error": "Direct plot actions may only be used on your turn."}

    action_type = str(data.get("action_type") or "").strip()
    try:
        gs, result = submit_plot_action(gs, player_id=mp.id, action_type=action_type, payload=data)
    except ValueError as exc:
        emit("error", {"message": str(exc)})
        return {"ok": False, "error": str(exc)}
    ...
    gs = log_and_broadcast(
        gs,
        "plot_action",
        result.get("summary") or f"Plot action '{action_type}' resolved.",
        match_id,
        redis_client,
        socketio,
        player_id=mp.id,
...
@socketio.on("plot_join")
def handle_plot_join(data):
    user_id = session.get("user_id")
    if not user_id:
        return {"ok": False, "error": "You must be signed in to respond to plot recruitment."}
    ...
    mp = MatchPlayer.query.filter_by(match_id=match_id, user_id=user_id).first()
    if not mp or mp.is_bankrupt:
        return {"ok": False, "error": "You cannot use plot membership actions in this match."}

    try:
        gs, result = submit_plot_join(gs, player_id=mp.id, payload=data)
    except ValueError as exc:
        emit("error", {"message": str(exc)})
        return {"ok": False, "error": str(exc)}
    ...
    gs = log_and_broadcast(
        gs,
        "plot_membership",
        result.get("summary") or "A plot membership action resolved.",
        match_id,
        redis_client,
        socketio,
        player_id=mp.id,
...
@socketio.on("plot_leave")
def handle_plot_leave(data):
    user_id = session.get("user_id")
    if not user_id:
        return {"ok": False, "error": "You must be signed in to leave the communist plot."}
    ...
    mp = MatchPlayer.query.filter_by(match_id=match_id, user_id=user_id).first()
    if not mp or mp.is_bankrupt:
        return {"ok": False, "error": "You cannot leave the communist plot in this match."}

    try:
        gs, result = submit_plot_leave(gs, player_id=mp.id)
    except ValueError as exc:
        emit("error", {"message": str(exc)})
        return {"ok": False, "error": str(exc)}
    ...
    gs = log_and_broadcast(
        gs,
        "plot_membership",
        result.get("summary") or "A player left the communist plot.",
        match_id,
        redis_client,
        socketio,
        player_id=mp.id,
...
@socketio.on("plot_counter_action")
def handle_plot_counter_action(data):
    user_id = session.get("user_id")
    if not user_id:
        return {"ok": False, "error": "You must be signed in to use coalition counter-actions."}
    ...
    mp = MatchPlayer.query.filter_by(match_id=match_id, user_id=user_id).first()
    if not mp or mp.is_bankrupt:
        return {"ok": False, "error": "You cannot use coalition counter-actions in this match."}

    action_type = str(data.get("action_type") or "").strip()
    if action_type != "join_coalition" and gs.get("current_player_id") != mp.id:
        emit("error", {"message": "Coalition counter-actions may only be used on your turn."})
        return {"ok": False, "error": "Coalition counter-actions may only be used on your turn."}

    try:
        gs, result = submit_plot_counter_action(gs, player_id=mp.id, action_type=action_type, payload=data)
    except ValueError as exc:
        emit("error", {"message": str(exc)})
        return {"ok": False, "error": str(exc)}
    ...
    gs = log_and_broadcast(
        gs,
        "plot_counter_action",
        result.get("summary") or f"Coalition action '{action_type}' resolved.",
        match_id,
        redis_client,
        socketio,
        player_id=mp.id,

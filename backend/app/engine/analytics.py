...
def _policy_metadata_map(policy_records: list[Any] | None = None) -> dict[str, dict]:
    metadata = {
        definition["target"]: dict(definition)
        for definition in get_default_lobbying_policies()
    }
...
def initialize_lobbying_stats(players: list[dict], policy_records: list[Any] | None = None) -> dict:
    metadata_map = _policy_metadata_map(policy_records)
    return {
        "player_totals": {
...
def sync_lobbying_policy_pool(game_state: dict, policy, entries: list[Any]) -> dict:
    policy_definition = get_lobbying_policy_definition(target_stat=policy.target_stat) or {}

...
def calculate_effective_build_loan_payout(base_payout: float, game_state: dict | None, econ: dict | None) -> float:
    return round_money(float(base_payout or 0) * build_loan_bonus_multiplier(game_state, econ))
...
def spend_investment_escrow(
    ...
    *,
    ...
    tranche_max_payout = calculate_effective_build_loan_payout(tranche_max_payout, next_state, econ)
...
def apply_investment_profit_share(
    ...
    *,
    ...
    triggered_rent_amount: float | None = None,
    owner_funded: bool = False,
    ...
    rent_value = round_money(triggered_rent_amount if triggered_rent_amount is not None else actual_rent_paid)
    if not tranches or rent_value <= 0:
        return next_state, []

    logs = []
    distributable = round_money(rent_value if owner_funded else actual_rent_paid)
    if distributable <= 0:
        return next_state, []
    ...
    for tranche in sorted(tranches, key=lambda entry: entry.created_at or datetime.utcnow()):
        ...
        incremental_rent = max(0.0, round_money(rent_value - float(tranche.baseline_rent or 0)))
        ...
        logs.append({
            ...
            "description": (
                f"${investor_share:.2f} of build-loan repayment was routed from property #{property_id}."
                if owner_funded
                else f"${investor_share:.2f} of rent from property #{property_id} was routed to an investor under an active deal."
            ),
            "investor_id": tranche.investor_id,
            "recipient_id": tranche.recipient_id,

...
def current_government_type(game_state: dict | None) -> str:
    game_state = game_state or {}
    settings = game_state.get("settings") or {}
    econ = game_state.get("econ") or {}
    return normalize_government_type(
...
def normalize_trade_lobby_pledges(
    match_id: int,
    raw_pledges: Any,
    ...
    for item in pledge_items:
        ...
        if policy_error:
            if "current government" in policy_error.lower():
                return [], "That lobbying pledge target is not available for the current government."
...
def find_unavailable_lobby_pledge_target(pledges: list[dict] | None, government_type: str | None) -> dict | None:
    for pledge in normalize_stored_lobby_pledges(pledges):
        if get_lobbying_policy_definition(
...
def normalize_trade_request_payload(
    ...
    data = data or {}
    government_type = current_government_type(game_state)
...
def validate_trade_proposal(game_state: dict, initiator_id: int, payload: dict) -> str | None:
    ...
    if has_lobby_pledges and (
        ...
        or government_type == "minarchism"
    ):
        return "Lobbying pledges are unavailable in the current government."
    if has_lobby_pledges and (
        find_unavailable_lobby_pledge_target(payload.get("offered_lobby_pledges"), government_type)
        or find_unavailable_lobby_pledge_target(payload.get("requested_lobby_pledges"), government_type)
    ):
        return "This trade includes lobbying pledges that are not available under the current government."
...
def apply_trade_acceptance(game_state: dict, trade, match_id: int) -> tuple[dict, dict | None, list[dict], str | None]:
    ...
    government_type = current_government_type(next_state)
    ...
    if has_lobby_pledges and (
        find_unavailable_lobby_pledge_target(offered_lobby_pledges, government_type)
        or find_unavailable_lobby_pledge_target(requested_lobby_pledges, government_type)
    ):
        return next_state, None, [], "This trade includes lobbying pledges that are not available under the current government."

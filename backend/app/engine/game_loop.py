...
from app.engine.plot import check_plot_victory
...
def initialize_government(gov_type: str) -> dict:
    ...
    if gov_type == "minarchism":
        ...
        return ensure_regime_economy_state({
            ...
            "welfare_payout": round(random.uniform(12, 24), 2),
            "tax_multiplier": round(random.uniform(0.18, 0.28), 3),
            "stability": 0.74,
...
def initialize_game_state(match, match_players, redis_client, socketio_instance) -> dict:
    ...
    gov_data = ensure_regime_economy_state(initialize_government(gov_type), settings)
    gov = Government(
        match_id=match.id,
    ...
    seeded_policies = seed_default_lobbying_policies(match.id, gov_type)
...
def load_game_state(match_id: int, redis_client) -> dict | None:
    ...
    game_state["econ"] = ensure_regime_economy_state(game_state.get("econ", {}), game_state.get("settings", {}))
    ...
    if not (game_state.get("lobbying_stats") or {}).get("policy_pools"):
        ...
        game_state["lobbying_stats"] = initialize_lobbying_stats(game_state.get("players", []), policy_records, government_type)
...
def resolve_lobbying(game_state: dict, econ: dict, settings: dict, socketio_instance, match_id: int) -> tuple[dict, dict]:
    ...
    for policy in policies:
        ...
        success_chance = calculate_lobbying_success_chance(
            ...
            government_type=econ.get("gov_type", settings.get("government_type", "liberal_democracy")),
...
def end_turn(
    ...
    if next_idx <= current_idx and len(turn_order) > 1:
        ...
        plot_winner = check_plot_victory(game_state)
        if plot_winner:
            game_state = dict(game_state)
            game_state["status"] = "completed"
            game_state["winner"] = plot_winner
            game_state.pop("awaiting_end_turn_player_id", None)
            game_state["dice_rolled_this_turn"] = False
            socketio_instance.emit("game_over", {"match_id": match_id, "winner": plot_winner}, room=str(match_id))
...
def _attempt_player_bailout(
    ...
    if gov_type == "liberal_democracy":
        recent_bailout_count = sum(
            1
            for entry in (((game_state.get("social") or {}).get("bailout_history") or []))
            if int(game_state.get("current_round", 1) or 1) - int(entry.get("round", game_state.get("current_round", 1)) or game_state.get("current_round", 1)) <= 4
        )
        repeat_penalty = max(0.0, recent_bailout_count - 1) * 1.25
        confidence_penalty = min(14.0, 2.5 + (bailout_amount / 400.0) + repeat_penalty)
        econ["market_confidence"] = round(max(5.0, float(econ.get("market_confidence", 70) or 70) - confidence_penalty), 2)
        econ = ensure_regime_economy_state(econ, settings)
    else:
        recent_bailout_count = 0
    ...
    game_state = log_and_broadcast(
        ...
        (
            ...
            + (
                f" Investor mood fell by {confidence_penalty:.1f} points"
                f" after {recent_bailout_count + 1} bailout{'' if recent_bailout_count + 1 == 1 else 's'} in the recent window."
                if gov_type == "liberal_democracy"
                else ""
            )
        ),
    ...
    socketio_instance.emit(
        "player_bailed_out",
        {
            ...
            "recent_bailout_count": recent_bailout_count + 1 if gov_type == "liberal_democracy" else 0,
            "match_id": match_id,
        },
        room=str(match_id),
    )

...
from app.engine.deals import attach_deals_snapshot, spend_investment_escrow
...
def _deal_scope_matches_property(scope: dict | None, prop: dict | None, owner_id: int) -> bool:
    if prop is None or int(prop.get("owner_id") or 0) != int(owner_id or 0):
        return False
...
def _estimate_owner_rent_threat(owner_id: int, game_state: dict) -> float:
    ...
    for prop in game_state.get("properties", []):
        if int(prop.get("owner_id") or 0) != int(owner_id) or prop.get("is_mortgaged"):
            continue
...
def build_deal_summary(player: dict, game_state: dict, profile: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if balance < reserve_floor * 1.25 and any(
        prop.get("property_type") == "property" and has_full_monopoly(prop, game_state) and int(prop.get("owner_id") or 0) == player_id
        for prop in game_state.get("properties", [])
    ):
...
def _social_lobby_target(entry: dict) -> str | None:
    targets = GRIEVANCE_POLICY_TARGETS.get(entry.get("dominant_grievance") or "", [])
    if not targets:
        return None
...
def choose_property_purchase(player: dict, prop: dict, game_state: dict, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    conservative_buy = value >= price * buy_value_margin and post_purchase >= reserve
    leverage_buy = (
...
def _score_deal_clause_for_player(player: dict, clause: dict, game_state: dict, profile: dict[str, Any]) -> float:
    ...
    if clause_type == "development_investment":
        ...
        if int(clause.get("beneficiary_id") or 0) == player_id:
            ...
            return (escrow_amount * 0.72) - max(0.0, max_payout - escrow_amount) * 0.18 + relief_bonus
        if int(clause.get("grantor_id") or 0) == player_id:
            ...
            return (max(0.0, max_payout - escrow_amount) * return_bias) - (escrow_amount * 0.62)
...
def choose_lobbying_move(player: dict, game_state: dict, profile: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if target is None and doctrine in {"policy_shaping", "social_democracy_leverage", "treasury_rebuild_then_leverage"} and regime["government_type"] == "social_democracy":
...
def choose_deal_proposal(player: dict, game_state: dict, profile: dict[str, Any]) -> dict[str, Any] | None:
    ...
    want_investment = bool(settings.get("private_equity_enabled", True)) and bool(buildable_props) and regime["investment_opportunity_score"] >= 0.32
    ...
    for other in game_state.get("players", []):
        ...
        if not clauses and want_investment and float(other.get("balance", 0) or 0) >= 250.0:
            ...
            clauses.append({
                ...
                "config": {
                    ...
                    "max_payout": round(escrow_amount * 1.45, 2),
                },
        ...
        proposal = {
            ...
            "title": "Short-term risk pact" if clauses[0]["type"] != "development_investment" else "Private equity bridge",
            "clauses": clauses,
        }
        ...
        if my_score >= 6.0 and other_score >= -10.0 and total_score > best_score:

...
from app.engine.plot import COMMUNIST_PLOT_OWNER_ID, calculate_solidarity_levy  # noqa: E402
...
class EventEngineTests(unittest.TestCase):
    ...
    def test_plot_seized_property_collects_solidarity_levy(self):
        redis_client = FakeRedis()
        socket = FakeSocket()
        player = {
            'id': 1,
        ...
        game_state = {
            'current_round': 6,
            'players': [dict(player), former_owner],
            'properties': [seized_property],
            'pending_debts': [],
            'free_parking_pot': 0.0,
            'social': {
                'plot': {
        ...
        econ = {'treasury_balance': 300.0, 'welfare_payout': 0.0, 'tax_multiplier': 0.15}
        settings = {'free_parking_pot_enabled': False}

        levy_info = calculate_solidarity_levy(seized_property, game_state)
        updated_player, updated_state, _updated_econ, logs = events.resolve_space(
            player,
            game_state,
            econ,
            settings,
        ...
        self.assertGreater(levy_info['amount'], 0)
        self.assertAlmostEqual(updated_player['balance'], 1000.0 - levy_info['amount'], places=2)
        self.assertEqual(updated_state['social']['properties']['1']['plot_last_levy_round'], 6)
        self.assertTrue(any(entry['event_type'] == 'solidarity_levy_collected' for entry in logs))
        rent_events = [payload for event, payload, _room in socket.emits if event == 'rent_collected']
        self.assertEqual(len(rent_events), 1)
        self.assertTrue(rent_events[0]['is_plot_levy'])
        self.assertEqual(rent_events[0]['owner_id'], COMMUNIST_PLOT_OWNER_ID)
        self.assertAlmostEqual(rent_events[0]['amount'], levy_info['amount'], places=2)
